...
Notice: Above, \texttt{\~{ }a} (and \code{"a"}) means that ``a'' has no parents while
...
A sparse matrix representation is obtained with:
<<print=T>>=
...
\begin{itemize}
\item The function \code{is.UG()} checks if the adjacency
matrix is symmetric (If applied to a graphNEL, the adjacency matrix is
created and checked for symmetry.)

\item The function \code{is.TUG()} checks if the graph is undirected and
...
\item The function \code{is.DG()} checks if a graph is directed, i.e., that
...
\item The function \code{is.DAG()} will
...
Coercion is generally faster to with \code{coerceGraph()}:
@
<<>>=
...
%% If speed is an issue, a faster coercion still is

%% @
%% <<>>=
%% if( require(microbenchmark) ){
%% microbenchmark( as(ug11, "matrix"), coerceGraph(ug11, "matrix"),
%%                graphNEL2matrix(ug11))
%% }
%% @ %def
...
Default is that the output representation is the same as the input
representation, but this can be changed:
@
<<>>=
...
\subsection{Perfect ordering, maximum cardinality search and triangulation}
